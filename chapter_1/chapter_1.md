...
* Exercise 1.20
  ...
  * With normal-order evaluation we have...
    ...
    ```
    ...until eventually we get to an expression consisting of bunch of nesting of `remainder` operations, where that expression evaluates to `0`. The thing to note here is that all the `remainder` operations that are performed are those in the "final" `if` statement, and then all the `remainder` operations actually constituting the GCD calculation once the predicate of the `if` is true. Doing a quick recurrence relation, we can see that if given inputs `a` and `b` to `gcd` are comprised of $n_{a_s}$ and $n_{b_s}$ `remainder` operations, respectively (at step $s$ in the recursion), then the arguments to the next recursive call of `gcd` will have $n_{a_{s + 1}} = n_{b_{s}}$ and $n_{b_{s + 1}} = n_{a_{s}} + n_{b_{s}} + 1$ `remainder` operations, respectively. (Note that this looks very similar to a Fibonacci recurrence!)
